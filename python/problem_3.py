...
def largest_prime_factor(n):
    ...
    # only goes up to sqrt(n) -
    # there is at most one prime factor, p2, greater than sqrtn
    s = list(
        filter(
            lambda x: n %
            x == 0,
    ...
    # all factors are either prime or composite,
    # filter wrt other integers not needed
    # last number left will be greatest prime in the list
    while len(s) > 1:
        i = s[0]
        s = list(filter(lambda x: x % i != 0, s))
        # for numbers of the form i^m,
        # s immediately becomes empty
        if not s:
            return i
    return s[0]
